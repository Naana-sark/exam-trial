...
if (isset($_POST["update"])){
    ...
    $up="UPDATE `judges` SET `fname`='$fname',`lname`='$lname',`case_id`= '$case_id',
    `contactnum1`='$contact',`gender`='$gender'  WHERE `judge_id`='$id'";
    // echo $up;
    // return $up;

    if($db->query($up)){
        echo "Updated";
        header('location: judgeindex.php');
    }
    else{
        echo "\nError: ". $db->error;
    }
...
<div  class="container"id="form-special">
       <h3 style="text-align:center;">Update Judge Form</h3>
        <form id = "update_form" action="updatejudge.php" method="POST">
